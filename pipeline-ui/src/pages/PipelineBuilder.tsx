...
function PipelineBuilder() {
  ...
  const handleRun = async (projectId: string) => {
    ...
    const jobId = await runPipeline(feature, finalProjectId);
    if (jobId) {
  ...
  const handleAddAgent = (agentName: string) => {
    if (customConfig.agents.some((a: any) => a.agent === agentName)) {
  ...
  const handleUpdateNode = (nodeId: string, updates: Partial<PipelineNode>) => {
    ...
    const agentIndex = customConfig.agents.findIndex((a: any) => a.agent === nodeId);
    if (agentIndex !== -1) {
  ...
  const handleDeleteNode = (nodeId: string) => {
    ...
    setCustomConfig({
      ...customConfig,
      agents: customConfig.agents.filter((a: any) => a.agent !== nodeId),
      edges: customConfig.edges?.filter((e: any) => e.source !== nodeId && e.target !== nodeId),
    });
  };
